...
`open()` 和 `close()` 是显式配对方法，用于嵌套场景。`close()` 在释放连接的同时，还会将 `this.query` 和 `this.orm` 置为 `null`：
...
public <TResult> TResult tx(ThrowingFunction<Orm, TResult, SQLException> func, Runnable callback) {
    ...
    try {
        ...
        this.commit();                    // 业务成功 → 提交
        callback.run();                   // 后置回调（在 commit 之后执行）
        return result;
...
- `commit()` 现在增加了空连接、autoCommit、isClosed 三项安全检查
- `rollback(Savepoint)` 增加了空连接安全检查（null 连接时直接 return）
- `setSavepoint()` / `setSavepoint(String)` 增加了空连接安全检查
- `tx()` 中不要再嵌套调用 `tx()`（不会报错但语义冗余）

### 数据库类型判断

`isSqlServer()` 和 `isMySql()` 现通过抽取的私有方法 `getDbType()` 统一获取数据库类型，消除重复的 `getDbConfig().getDbType()` 调用：
...
`createOrm()` 现在**缓存 Orm 实例**（惰性创建），多次调用返回同一对象：
...
`close()` 在释放连接时将 `this.orm` 置为 `null`，下次 `createOrm()` 重新创建。
...
new Query(db, sql, autoPK)
  → createQuery(sql, autoPK)     // 重置状态→关闭旧 PS→设置新 SQL/PK
  → 首次执行时 createPreparedStatement()
     → connection.prepareStatement(sql, 或 RETURN_GENERATED_KEYS)
  → close()                       // 设置 closed=true，关闭 PreparedStatement
```

**关闭保护：** `close()` 设置 `closed = true` 标志。所有 public 方法（`find`、`get`、`execute`、`executeBatch`、`addBatch`、`addParameter/s`）在操作前调用 `checkNotClosed()` 检查，已关闭的 Query 直接抛出 `DataRetrievalException`：
...
所有 public 方法（`find`、`get`、`execute`、`executeBatch`、`addBatch`、`addParameter/s`）在操作入口调用 `checkNotClosed()`。

### 参数序号验证

`addParameter(int paramIndex, Object param)` 增加了参数序号合法性检查，`paramIndex` 必须 `>= 1`：
...
**时间统计条件守卫：** `find()` 中的耗时统计代码现在通过 `sqlLog.isShowResultEnabled()` 控制，仅在启用结果日志时才计时：
...
private boolean closed;               // Query 关闭状态标记
```

### closePreparedStatement() 与 acceptKeysRunnable()

`closePreparedStatement()` 在 `finally` 块中将 `this.preparedStatement` 置为 `null`，确保异常后引用被清除：
...
`acceptKeysRunnable()` 的时间统计已修复，统一使用 `System.nanoTime()` 计算耗时（此前混用了 `currentTimeMillis` 和 `nanoTime`）：
...
| **合计** | **单元测试总数** | **465+** | 含 Cond、Sql、Snowflake、ReflectUtil、Order、TableAlias 等 |
