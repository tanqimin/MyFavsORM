...
public class Database
  ...
  public Database(Orm orm) {
    ...
    this.orm = orm;
    this.dialect = DialectFactory.getInstance(this.orm.getDbType());
    this.sqlLog = new SqlLog(this.orm.getShowSql(), this.orm.getShowResult());
  }
  ...
  private Connection getConnection() {
    ...
    return this.orm.createConnection();
  }
  ...
  public <TView> List<TView> find(Class<TView> viewClass, String sql, List<Object> params) {
    ...
    try {
      ...
      pstmt.setFetchSize(this.orm.getFetchSize());
      rs = pstmt.executeQuery();

      result = DBConvert.toList(viewClass, rs);

      this.sqlLog.showResult(rs);
      return result;
  ...
  public <TView> PageLite<TView> findPageLite(Class<TView> viewClass, String sql, List<Object> params, boolean enablePage, int currentPage,
    ...
    pagSize = pageSize;
    if (enablePage) {
      long maxPageSize = this.orm.getMaxPageSize();
...
//    }
  ...
  public <TModel> int create(Class<TModel> modelClass, TModel entity) {
    ...
    */
    ...
    try {
      this.sqlLog.showSql(sql.getSql().toString(), sql.getParams());
  ...
  public <TModel> int create(Class<TModel> modelClass, Collection<TModel> entities) {
    ...
    try {
      this.sqlLog.showBatchSql(sql.getSql().toString(), paramsList);

      conn = getConnection();
      pstmt = DBUtil.getPsForUpdate(conn, autoGeneratedPK, sql.getSql().toString(), paramsList, this.orm.getBatchSize());
  ...
  public <TModel> int update(Class<TModel> modelClass, Collection<TModel> entities, String[] columns) {
    ...
    try {
      ...
      conn = getConnection();
      pstmt = DBUtil.getPsForUpdate(conn, false, sql.getSql().toString(), paramsList, this.orm.getBatchSize());
