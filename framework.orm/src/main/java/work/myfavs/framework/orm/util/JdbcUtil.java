...
public class JdbcUtil {
   ...
   * @throws SQLException SQLException
   */
  public static PreparedStatement getPstForQuery(Connection conn, String sql, Collection<?> params) throws SQLException {

    final PreparedStatement pst = getPstForQuery(conn, sql);
    return addParameters(pst, params);
  }

  public static PreparedStatement getPstForUpdate(Connection conn, boolean autoGeneratedPK, String sql, Collection<?> params) throws SQLException {

    final PreparedStatement pst = getPstForUpdate(conn, autoGeneratedPK, sql);
    return addParameters(pst, params);
  }

  public static PreparedStatement getPstForUpdate(Connection conn, boolean autoGeneratedPK, String sql) throws SQLException {
    if (autoGeneratedPK) {
  ...
  public static int executeBatch(PreparedStatement pst, Collection<Collection<?>> params, int batchSize) throws SQLException {
    int result = 0, execIdx = 0;
    for (Collection<?> param : params) {
      addParameters(pst, param);
      pst.addBatch();
      execIdx++;
   ...
   * @throws SQLException SQLException
   */
  private static PreparedStatement addParameters(PreparedStatement preparedStatement, Collection<?> params) throws SQLException {
  ...
  public static void addParameter(PreparedStatement preparedStatement, int index, Object param) throws SQLException {
    if (Objects.isNull(param)) {
      preparedStatement.setObject(index, null);
    } else {
      //noinspection unchecked
      PropertyHandlerFactory.getInstance(param.getClass()).addParameter(preparedStatement, index, param);
    }
  }
  ...
  public static void close(ResultSet resultSet, Statement statement, Connection connection) {
    close(resultSet, statement, () -> JdbcUtil.close(connection));
  }

  public static void close(ResultSet resultSet, Statement statement, ThrowingRunnable<Exception> connectionRunnable) {
    try {
      close(resultSet);
    } catch (SQLException e) {
