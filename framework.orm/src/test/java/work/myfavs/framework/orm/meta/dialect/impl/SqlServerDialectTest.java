...
//  private static final String CASE1 = "SELECT * FROM TABLE_NAME";
  ...
  public void selectTop() {
    ...
    Assert.equals(
        selectTop2.toString(), "SELECT TOP 3 * FROM TABLE_NAME ORDER BY col1");
    Assert.equals(
        selectTop3.toString(),
  ...
  public void selectPage() {
    ...
    Assert.equals(
        ...
        "SELECT * FROM ( SELECT *, ROW_NUMBER() OVER (ORDER BY CURRENT_TIMESTAMP) AS _rn FROM TABLE_NAME ) _paginate WHERE _rn > 10 AND _rn <= 20");
    Assert.equals(
        selectPage2.toString(),
        "SELECT * FROM ( SELECT *, ROW_NUMBER() OVER (ORDER BY col1) AS _rn FROM TABLE_NAME ) _paginate WHERE _rn > 20 AND _rn <= 30");
    Assert.equals(
        selectPage3.toString(),
        "WITH QUERY AS ( SELECT * FROM TABLE_NAME ) SELECT * FROM ( SELECT *, ROW_NUMBER() OVER (ORDER BY id) AS _rn FROM QUERY WHERE col1 = ? ) _paginate WHERE _rn > 60 AND _rn <= 80");
    Assert.equals(
            selectPage4.toString(),
            "SELECT * FROM ( SELECT *, ROW_NUMBER() OVER (ORDER BY id) AS _rn FROM ( SELECT * FROM TABLE_NAME ) QUERY WHERE col1 = ? ) _paginate WHERE _rn > 60 AND _rn <= 80");
