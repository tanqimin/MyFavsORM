...
public abstract class AbstractOrm implements Orm {
    ...
    */
    ...
    public <TModel> int create(Class<TModel> modelClass, Collection<TModel> entities) {
        ...
        if (isIdentity) {
            /*
             * SQL Server JDBC executeBatch() + getGeneratedKeys() 存在已知 bug，
             * 改用 OUTPUT INSERTED 子句构建多行 INSERT 来获取自增 ID。
             * 参考：https://stackoverflow.com/questions/13641832
             */
        ...
        /*
         * 非自增主键（SNOW_FLAKE/UUID/ASSIGNED）统一走多行 VALUES 批量 INSERT，
         * 对所有数据库通用且性能优于逐条 JDBC Batch。
         */
        return createInSqlBatch(classMeta, entities);
    }

    /*
     * SQL Server 专用：使用 OUTPUT INSERTED 子句构建多行 INSERT 并获取自增 ID。
     *
     * SQL Server JDBC 的 executeBatch() + getGeneratedKeys() 存在已知 bug，
     * 无法通过 JDBC Batch 获取自增主键。此方法绕过该问题：
     * 1. 构建 INSERT INTO ... OUTPUT INSERTED.pkCol VALUES (...), (...), ... 语句
     * 2. OUTPUT INSERTED 使 INSERT 返回结果集，可用 executeQuery() 读取
     * 3. 将返回的 identity 值按 VALUES 顺序设置回对应实体的主键字段
     * 参考：https://stackoverflow.com/questions/13641832
     */
