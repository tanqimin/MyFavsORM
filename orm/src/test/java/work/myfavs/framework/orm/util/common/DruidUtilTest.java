...
public class DruidUtilTest {
  ...
  @Test
  public void formatSqlShouldHandleComplexQueries() {
    ...
    assertTrue(sqlStatements.get(0) instanceof SQLSelectStatement);
    assertNotNull(sqlStatements.get(0).toParameterizedString());
    ...
    assertNotNull(formatSql);
    assertTrue(formatSql.contains("sku_id"));
    assertTrue(formatSql.contains("product_code"));
    assertTrue(formatSql.contains("brand_name"));
    assertTrue(formatSql.toUpperCase().contains("LEFT JOIN"));
  }
}
