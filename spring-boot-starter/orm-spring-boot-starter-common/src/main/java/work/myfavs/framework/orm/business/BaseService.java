...
/**
 * 事务基类，提供基于 {@link PlatformTransactionManager} 的编程式事务控制.
 * <p>支持有返回值（{@link TransactionCallback}）和无返回值（{@link Consumer}）两种事务模式，
 * 可指定事务隔离级别、超时时间和只读属性.</p>
 *
 * @author tanqimin
 */
public abstract class BaseService {
   ...
   * @param <T>      返回值类型
   * @param callback 事务回调
   * @param timeout  超时时间（秒），{@code -1} 表示使用默认值
   * @return 回调执行结果
   */
